...
class CutIn(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        '''
        1.行为树中的每个节点都需要向其父节点返回一个值，以允许父节点根据子节点的运行情况继续运行
        2.每棵树都有一个根节点，这个节点没有特殊的意义
        
        Composite Node
        1.Slector:实现子节点 或 关系，按定义顺序遍历子节点，直到有一个子节点返回true时停止，并返回true。
                  如果子节点全部返回false，则返回false。
        2.RandomSelector:类似Selector，不同之处在于，按照随机顺序遍历子节点。
        3.Sequence:实现子节点 与 关系，按定义顺序遍历子节点，直到有一个节点返回false时停止，并返回false。
                   如果子节点全部返回true，则返回true。
        ...
        2.Failure:拥有一个子节点，执行子节点后，返回false
        3.Negate:拥有一个子节点，返回子节点返回值的相反值(true变false, false变true)
        ...
        car_visible = ActorTransformSetter(self.other_actors[0], self._transform_visible)  # 车辆生成位置
        ...
        car_driving = WaypointFollower(self.other_actors[0], self._velocity)  # 航路点跟车的速度
        just_drive.add_child(car_driving)

        trigger_distance = InTriggerDistanceToVehicle(
            self.other_actors[0], self.ego_vehicles[0], self._trigger_distance)  # 场景触发距离
